		        ...
		        },*/
		        {
		            ...
		            label: {
		                ...
		                formatter(params) {
		                    ...
		                    let arr = [params.name, "社区：" + params.value[1] , "用户：" + params.value[0]];
		                    return arr.join("\n")
		                },
		                textStyle: {
		                    align: 'left',
...
function wuran(){
	...
	var salvProName =["北京市望京社区","上海浦东社区","深圳罗湖社区","北京丰台社区","杭州西湖社区"];
...
function zhexian() {
	...
	dataObj = {
		year: ['3.05', '3.06', '3.07', '3.08', '3.09', '3.10'],
		data: {
			value: [{
				name: '完成',
				value: [65, 85, 90, 99, 204, 333]
			}, {
				name: '审核',
				value: [70, 85, 116, 99, 206, 355]
			}, {
				name: '发布',
				value: [74, 85, 120, 100, 206, 355]
			}]
		}
	}
	let dataArr = []
	...
	dataObj.data.value.map(item => {
		...
		let newArr = {
			...
			itemStyle: {
				normal: {
					color: item.name === '完成' ? '#f0c725' : item.name === '审核' ? '#0BE3FF' : '#16f892'
				}
			}
		}
